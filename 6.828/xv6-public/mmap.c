...
{
  // Your code here.
  static double *sqrts_pg = NULL;
  // replace these three lines with your implementation
  uintptr_t fault_addr = (uintptr_t)si->si_addr;
  if(sqrts_pg && munmap(sqrts_pg, page_size)<0)
    fprintf(stderr, "Error unmap : %s\n", strerror(errno));
  sqrts_pg = mmap((void*)(align_down(fault_addr, page_size)), page_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  calculate_sqrts(sqrts_pg, sqrts_pg - sqrts, page_size / sizeof(double));
